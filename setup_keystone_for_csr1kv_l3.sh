#!/bin/bash
# Change log:
# 11-June-2015: Hareesh Changes to use openstack CLI instead of keystone CLI
...
tenantId=`openstack project show $l3AdminTenant 2>&1 | awk '/No tenant|id/ { if ($3 == "No") print "No"; else if ($2 == "id") print $4; }'`
...
userId=`openstack user show $regularUser 2>&1 | awk '/No user|id/ { if ($3 == "No") print "No"; else print $4; }'`
if [ "$userId" == "No" ]; then
   echo " No, it does not. Creating it."
...
tenantId=`openstack project show $serviceTenant 2>&1 | awk '/No tenant|id/ { if ($3 == "No") print "No"; else if ($2 == "id") print $4; }'`
